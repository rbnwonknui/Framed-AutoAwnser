...
(function() {
  ...
  async function fetchMovieData(day) {
      try {
          const url = `https://titles.framed.wtf/v1/titles/top-ten/${day}?gameType=one-frame`;
  ...
  function collectFramedDay(day) {
      ...
      showNotif('System started', `Day ${day} detected`, 'info', async () => {
          try {
              ...
              if (movieData?.frames && movieData.frames.length > 0) {
                  const lastFrame = movieData.frames[movieData.frames.length - 1];
                  
                  if (lastFrame?.items) {
                      try {
                          const items = JSON.parse(lastFrame.items);
                          
                          if (items && items.length > 0) {
                              title = items[0].title;
                              console.log(`🎬 Frame ${lastFrame.frame_number} - Most voted title: "${title}"`);
                          }
                      } catch (parseError) {
